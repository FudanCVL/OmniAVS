...
from .constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
                        ...
                        SIGLIP_MEAN, SIGLIP_STD, AUDIO_START_TOKEN, AUDIO_END_TOKEN, AUDIO_CONTEXT_TOKEN, SUB_AUDIO_START_TOKEN, SUB_AUDIO_END_TOKEN, SUB_AUDIO_CONTEXT_TOKEN)
...
def preprocess_mpt(
    ...
    if kwargs.get("audio_enable", False):
        assert kwargs.get("num_audio_token", None)
        num_audio_token = kwargs.get("num_audio_token", None)
        new_conversations = []
        if not isinstance(num_audio_token, list):
            num_audio_token = [num_audio_token]
        for conversation in conversations:
            if "sub_audio" in conversation:
                # Only use first audio input for sub_audio tokens
                total_length = num_audio_token[0]
                num_segments = conversation.count('<sub_audio>')
                ...
                for i in range(num_segments):
                    ...
                    audio_tokens = f'{SUB_AUDIO_START_TOKEN}{SUB_AUDIO_CONTEXT_TOKEN * current_length}{SUB_AUDIO_END_TOKEN}'
                    conversation = conversation.replace('<sub_audio>', audio_tokens, 1)
                
            for i in range(len(num_audio_token)):
                audio_tokens = f'{AUDIO_START_TOKEN}{AUDIO_CONTEXT_TOKEN * num_audio_token[i]}{AUDIO_END_TOKEN}'
                conversation = conversation.replace('<audio>', audio_tokens, 1)
            new_conversations.append(conversation)
        conversations = new_conversations
...
def preprocess_internlm(
    ...
    if kwargs.get("audio_enable", False):
        ...
        new_conversations = []
        for conversation in conversations:
            audio_tokens = f'{AUDIO_START_TOKEN}{AUDIO_CONTEXT_TOKEN * num_audio_token}{AUDIO_END_TOKEN}'
            conversation = conversation.replace('<audio>', audio_tokens, 1)
            new_conversations.append(conversation)
        conversations = new_conversations
    ...
    for conversation, target in zip(conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())  # pad_token_id = eos_token_id
        cur_len = 1
        target[:cur_len] = IGNORE_TOKEN_ID  # <s>
        parts = conversation.split(conv.roles[1])  # [UNUSED_TOKEN_146]assistant\n
        info = parts[0] + conv.roles[1]
        temp_len = len(tokenizer(info).input_ids) - 1  # tokenizer的<s>
