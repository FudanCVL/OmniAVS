...
class OmniAVSDataset(RVOSDataset):
    ...
    def sample_data(self, idx):
        ...
        if '<video>' not in data['conversations'][0]['value']:
            data['conversations'][0]['value'] = '<video><sub_audios>\n' + '<audio>\n' + data['conversations'][0]['value']
        special_tokens = '\n'.join(['Frame{}: <image><sub_audio>'.format(i + 1) for i in range(len(image_list))])
        data['conversations'][0]['value'] = data['conversations'][0]['value'].replace('<video><sub_audios>', special_tokens)
        ...
        ret = dict(
            ...
            audio_flags=torch.tensor([2] + [1] * (len(audio_list) - 1), dtype=torch.long),
