...
class AudioEncoder(nn.Module):
    ...
    def encode(self, input_audios: Tensor, input_audio_lengths: Tensor):
        real_input_audio_lens = input_audio_lengths[:, 0].tolist()
        max_len_in_batch = max(real_input_audio_lens)
        padding_mask = torch.ones([input_audios.size(0), max_len_in_batch]).to(dtype=self.conv1.weight.dtype,
                                                                               device=self.conv1.weight.device)
...
def process_audio(audio_url):
    ...
    input_audio_lengths = torch.IntTensor(audio_len)
    return {"input_audios": mel,
...
def _np_extract_fbank_features(waveform_batch: np.array, device="cpu", n_fft=400, hop_length=160) -> np.ndarray:
    """
    ...
    return log_spec_batch[0]


def process_audiov2(audio_url):
    # if len(audio_urls) > 0:
    # audios, audio_lens, audio_span_tokens = [], [], []
    # for audio_path in audio_urls:
    # audio = librosa.load(audio_url, sr=16000)[0]
    audio = load_audio(audio_url)
    # todo:
    L = (audio.shape[0] if audio.shape[0] <= 480000 else 480000)  # max_length < 30s
    mel_len = L // 160
    audio = pad_or_trim(audio.flatten())
    mel = _np_extract_fbank_features([audio])
    audio_len_after_cnn = get_T_after_cnn(mel_len)        # input_lengths
    audio_token_num = (audio_len_after_cnn - 2) // 2 + 1  # output_lengths
    audio_len = [audio_len_after_cnn, audio_token_num]
    # audios.append(mel)
    # audio_lens.append(audio_len)
    audio_span_tokens = audio_token_num + 2
    input_audio_lengths = torch.IntTensor(audio_len)
    return {
        "input_audios": torch.from_numpy(mel),
        "input_audio_lengths": input_audio_lengths,
        "audio_span_tokens": audio_span_tokens,
        "audio_urls": audio_url,
...
def process_audio_whisper(audio_url, chunk_size=480000):
    audio, _ = librosa.load(audio_url, sr=16000, mono=True)
    # todo:
    if audio.shape[0] < chunk_size:
        ...
        segments = []
        segment_length = chunk_size  # 30 seconds
        
        for start in range(0, len(audio), segment_length):
            end = start + segment_length
            segment = audio[start:end]
            original_length = len(segment) // 160
            if len(segment) < segment_length:
                segment = pad_or_trim(segment.flatten(), length=chunk_size)
            segments.append((segment, original_length))

        mel_segments = [_np_extract_fbank_features([segment]) for segment, _ in segments]
        audio_lens = [get_T_after_cnn(original_length) for _, original_length in segments]
        audio_token_nums = [(length - 2) // 2 + 1 for length in audio_lens]
        audio_span_tokens = [token_num + 2 for token_num in audio_token_nums]
        input_audio_lengths = torch.IntTensor([[length, token_num] for length, token_num in zip(audio_lens, audio_token_nums)])

        return {
            "input_audios": np.stack(mel_segments, axis=0),
            "input_audio_lengths": input_audio_lengths,
            "audio_span_tokens": audio_span_tokens,
            "audio_urls": audio_url,
            "audio_len_after_cnn": audio_lens,
            "audio_token_num": audio_token_nums
        }
        
# process_audiov2 = process_audio_whisper

if __name__ == '__main__':
    output = process_audio_whisper("examples/audio1.mp3")
    print(output)
